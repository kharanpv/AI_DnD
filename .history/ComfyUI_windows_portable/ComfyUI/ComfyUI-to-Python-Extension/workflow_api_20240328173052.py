...
def get_prompts_and_samples():
    ...
    json_path = os.path.join(script_dir, "request.json")
...
def main() -> None:
    ...
    with torch.inference_mode():
        ...
        print(prompts)
        file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "Custom_Scripts", "temp.json")
        ...
        cliptextencode_6 = cliptextencode.encode(
            text= 'A single ' + positive_prompt + ', white background',
            clip=get_value_at_index(loraloader_10, 1),
        )

        cliptextencode_7 = cliptextencode.encode(
            text='multiple' if negative_prompt == '' else negative_prompt,
            clip=get_value_at_index(loraloader_10, 1)
        )
