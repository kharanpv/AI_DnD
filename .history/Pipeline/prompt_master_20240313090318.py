...
# Add ComfyUI_windows_portable/ComfyUI to path
sys.path.append(os.path.join(script_dir, "..", "ComfyUI_windows_portable", "ComfyUI"))
...
def find_prompt():
    with open(chat_history_path, 'r') as file:
        chat_history = json.load(file)
        last_dm_response = None
        for item in reversed(chat_history):
            if "dm_response" in item:
                last_dm_response = item["dm_response"]
                break
        if last_dm_response:
...
def generate_response():
    ...
    try:
        subprocess.run(["python", img_pipeline_script], check=True) # TODO: Parse correct arguments
